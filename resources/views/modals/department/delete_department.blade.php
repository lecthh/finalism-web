<div id="delete-department-{{ $departmentId }}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <form action="{{ url('departments/' . $departmentId) }}" method="post">
        {{ csrf_field() }}
        {{ method_field('DELETE') }}
        <input type="hidden" name="deptid" id="deptidToDelete" value="{{ $departmentId }}">
        <div class="w-[500px] bg-white flex flex-col gap-y-6 p-6 justify-center items-center">
            <h1 class="font-bold text-2xl">Delete Department</h1>
            ...
            <button type="submit" data-modal-target="delete-department-{{$departmentId}}" data-modal-toggle="delete-department-{{$departmentId}}" class="bg-[#CFCED0] gap-x-2 px-4 py-3 hover:bg-[#767676] transition ease-in-out delay-100">
                <h1 class="text-white">Delete Department</h1>
            </button>
        </div>
    </form>
</div>
