<div id="delete-college-{{ $collegeId }}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <form action="{{ url('colleges/' . $collegeId) }}" method="post">
        {{ csrf_field() }}
        {{ method_field('DELETE') }}
        <input type="hidden" name="collid" id="collidToDelete" value="{{ $collegeId }}">
        <div class="w-[500px] bg-white flex flex-col gap-y-6 p-6 justify-center items-center">
            ...
            <button type="button" data-modal-target="delete-college-{{$collegeId}}" data-modal-toggle="delete-college-{{$collegeId}}" class="bg-[#FF0B47] gap-x-2 px-4 py-3 hover:bg-[#B90934] transition ease-in-out delay-100">
                <h1 class="text-white">Cancel</h1>
            </button>
            <button type="submit" data-modal-target="delete-college-{{$collegeId}}" data-modal-toggle="delete-college-{{$collegeId}}" class="bg-[#CFCED0] gap-x-2 px-4 py-3 hover:bg-[#767676] transition ease-in-out delay-100">
